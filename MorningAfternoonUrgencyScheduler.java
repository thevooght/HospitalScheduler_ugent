...
public class MorningAfternoonUrgencyScheduler extends BaseUrgencyScheduler {
    ...
    public void fill(Week w, int nUrgencySlots) {
        ...
        for (int i = 0; i < w.days.length; i++) {
            if (w.days[i].equals(DayOfWeek.THURSDAY) || w.days[i].equals(DayOfWeek.SATURDAY)) {
                w.days[i].timeSlots[16].reservedForUrgent = true;
            } else {
                w.days[i].timeSlots[16].reservedForUrgent = true;
                w.days[i].timeSlots[32].reservedForUrgent = true;
            }
        }
        ...
        if (nUrgencySlots > 10 && nUrgencySlots < 15) {
            ...
            for (int i = 10; i < nUrgencySlots; i++) {
                if (w.days[dayCounter].equals(DayOfWeek.THURSDAY)) {
        ...
        if (nUrgencySlots > 16) {
            ...
            for (int i = 16; i < nUrgencySlots; i++) {
                ...
                w.days[dayCounter].timeSlots[31].reservedForUrgent = true;
                dayCounter++;
            }
        }
